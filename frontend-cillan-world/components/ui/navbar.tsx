...
const Navbar = () => {
  ...
  const [isVisible, setIsVisible] = useState(false);         // Control de montaje
  const [isAnimatingOut, setIsAnimatingOut] = useState(false);
  const { t, i18n } = useTranslation();

  // Evita renderizar el modal hasta que las traducciones estén listas
  ...
  let result: CollectionType[] = useGetCollections().result ?? [];
  ...
  const openCart = () => {
    setIsVisible(true);
    requestAnimationFrame(() => setIsCartOpen(true)); // Asegura aplicación de clase CSS
  };
  ...
  return (
    <nav className="fixed top-0 left-0 w-full h-16 flex items-center justify-between px-4 z-50 bg-transparent">
              ...
              <ul className="text-white text-center space-y-1 text-2xl font-handwriting z-20 relative">
                ...
                {activeSubmenu === "shop" && (
                  <ul className="mt-1 space-y-0 text-md">
                    <li onClick={() => goToCatalog("view-all")} className="hover:uppercase cursor-pointer text-outline-black">{t("navbar.view_all")}</li>
                    <li onClick={() => goToCatalog("tops")} className="hover:uppercase cursor-pointer text-outline-black">{t("navbar.tops")}</li>
                    <li onClick={() => goToCatalog("bottoms")} className="hover:uppercase cursor-pointer text-outline-black">{t("navbar.bottoms")}</li>
                    <li onClick={() => goToCatalog("runaway-pieces")} className="hover:uppercase cursor-pointer text-outline-black">{t("navbar.runaway_pieces")}</li>
                  </ul>
                )}
                ...
                {activeSubmenu === "universe" && (
                  <ul className="mt-1 space-y-0 text-md max-h-36 overflow-y-auto">
                    {result.sort((a, b) => (a.order ?? 0) - (b.order ?? 0)).map((collection) => (
      ...
      {/* Botón derecha (carrito) y LanguageSwitcher juntos */}
      <div className="flex items-center gap-2">
        ...
        >
          <div className="relative w-24 h-6">
            <span className="block w-24 text-center text-[15px] font-semibold text-white text-outline-black tracking-wide mb-4">{t("bag.your_bag")}</span>
            {cartItems.length > 0 && (
      ...
      {/* Modal del carrito extraído a componente */}
      <CartModal
        isVisible={isVisible}
